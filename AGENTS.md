...
- **Product shape.** Tauri 2 desktop app for Windows and macOS; operator-maintained document control; dual roots (edit + publish) with mirrored relative trees; foldable left menu (hamburger when collapsed) with open-activity panes/tabs; library directory navigator (metadata list, selection pane with master data and document/batch actions) with add/remove; stable document permalinks (`workspace ID` + `document ID`); Office drafts; app-driven versioning + PDF export via preinstalled Microsoft Office to checksummed `*_VMAJOR.MINOR.pdf`; cosmetic changes increment minor and substantive changes increment major; `<edit-root>/.dms` metadata with no application database. Detail: `docs/architecture.md` and `docs/design-decisions.md`.
