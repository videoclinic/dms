...
| 1 | Shared Rust core + `dms` CLI foundation, then Tauri 2 app skeleton (Windows + macOS) + DOX for source tree | in-progress (`cargo fmt --check`, `cargo test --workspace`, and `cargo clippy --workspace --all-targets -- -D warnings` pass for the CLI slice; desktop skeleton remains) | `cargo test --workspace` covers explicit confirmed workspace initialization, stable IDs, in-root supported-source registration, independent document-control data, unique document numbers, and note CRUD; the later desktop portion launches on Windows and macOS and proves the phase's shell, activity, and saved-view requirements |
...
- `crates/dms-core` owns Tauri-independent domain behaviour. `crates/dms-cli`
  ...
  Entra, or mail dependency. A later desktop adapter must call the same core.
