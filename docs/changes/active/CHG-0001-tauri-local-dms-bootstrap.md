...
| 1 | Shared Rust core + `dms` CLI foundation, then Tauri 2 app skeleton (Windows + macOS) + DOX for source tree | in-progress (local Rust 1.88 and shell gates pass; macOS launch smoke passes; Windows requires its Tauri `.ico` resource before the platform gate can complete) | The workspace MSRV meets the selected Tauri 2 release; `cargo test --workspace` covers explicit confirmed workspace initialization, stable IDs, in-root supported-source registration, independent document-control data, unique document numbers, note CRUD, and desktop shell state; the desktop app launches on Windows and macOS and proves the phase's shell, activity, and saved-view requirements |
...
**Current phase:** 0e is complete; phase 1 is in progress. Keep only one phase in progress.
