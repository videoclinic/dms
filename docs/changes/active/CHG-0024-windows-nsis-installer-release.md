...
- Run 32169684243 **hung 60 min again — this time on the keyless export**, disproving the private-key/CAPI-keyset theory: `::new()` succeeded and the block is `.Add()` to the Root store itself, with or without a key, on headless `windows-latest`. Every other call is proven fast (`Import-PfxCertificate` into `My` ~300 ms, `Open('ReadWrite')` instant per run 3's instant `.Add` error, `::new` per run 7's instant failure). Final fix: trust via native **`certutil -addstore -user Root <cer>`** (file-based, synchronous, the standard CI path — no .NET store management); the private key stays in `My` where `signtool` needs it.
