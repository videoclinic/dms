...
| 5 | Winget submission kit: `Build-WingetKit.ps1` + workflow step + operator runbook | in-progress (upstream validation) | Run 32248202153 passed the stable-tag build/sign/release/render/upload path; its `winget-bundle` artifact (9363474625) has all three header-bearing 1.12 manifests and `SUBMIT.md`, and its installer hash matches the published `v0.1.0` sidecar. The same final bundle is attached to the release. Upstream PR [microsoft/winget-pkgs#420554](https://github.com/microsoft/winget-pkgs/pull/420554) is open with exactly those three manifests; the personal CLA and validations 01–07 passed. Gate: upstream installation validation, installer metadata validation, final completion, and review must pass; then `winget install Videoclinic.DMSDesktop --version 0.1.0` must work from the public source. |
...
- Run 32248202153 **passed** after the workflow was made tag-source-consistent and the artifact upload path was changed from `env.RUNNER_TEMP` to `${{ runner.temp }}`. It built and signed the `v0.1.0` installer, uploaded `winget-bundle` artifact 9363474625, and rendered the three required `Videoclinic.DMSDesktop` manifests plus `SUBMIT.md`. The artifact uses the upstream-recommended `ManifestVersion: 1.12.0` and required yaml-language-server schema headers; all three files passed the official winget 1.12 JSON schemas locally. The published installer download and `.sha256` sidecar match `d78de0ca7d5f3e720b550f83499c48fb1af151f56fb42c83daafb111969e8690`; the release-attached `winget-bundle.zip` was replaced from that final artifact.
- Submission PR [microsoft/winget-pkgs#420554](https://github.com/microsoft/winget-pkgs/pull/420554) is open from `forgegod/winget-pkgs:videoclinic-dmsdesktop-0.1.0` at commit `cd9c09626`, with exactly the three package manifests (46 additions, no other files). The account owner accepted Microsoft's CLA in issue comment 5341670579; `Needs-CLA` cleared and the PR is labelled `New-Package`. Validations 01–07 and the CLA passed. At the latest check, 08 Installation Validation was queued, with 09 Installer Metadata Validation and 10 Validation Completed behind it. Phase 5 remains in progress until upstream completes those checks and review.
