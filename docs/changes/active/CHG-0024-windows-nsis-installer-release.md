...
| 5 | Winget submission kit: `Build-WingetKit.ps1` + workflow step + operator runbook | in-progress (external CLA gate) | Run 32248202153 passed the stable-tag build/sign/release/render/upload path; its `winget-bundle` artifact (9363474625) has all three header-bearing 1.12 manifests and `SUBMIT.md`, and its installer hash matches the published `v0.1.0` sidecar. The same final bundle is attached to the release. Upstream PR [microsoft/winget-pkgs#420554](https://github.com/microsoft/winget-pkgs/pull/420554) is open with exactly those three manifests, but has the `Needs-CLA` label and queued validation jobs. Gate: the GitHub account owner must explicitly accept Microsoft's CLA; then upstream validation and review must pass and `winget install Videoclinic.DMSDesktop --version 0.1.0` must work from the public source. |
...
- Submission PR [microsoft/winget-pkgs#420554](https://github.com/microsoft/winget-pkgs/pull/420554) is open from `forgegod/winget-pkgs:videoclinic-dmsdesktop-0.1.0` at commit `cd9c09626`, with exactly the three package manifests (46 additions, no other files). Upstream requested the account owner's explicit Microsoft CLA agreement and applied `Needs-CLA`; the full validation jobs are queued behind it. This is an external legal gate, so Phase 5 remains in progress.
