...
- Runs 32159384160 + 32163986746 **hung for 27–50 min in the import step** before being cancelled (initially suspected saturated shared runners; both partial logs disprove that). Cross-referencing: runs 2/3/4 passed `Import-PfxCertificate` into `My` and `X509Store Root .Open('ReadWrite')` in well under a second — run 3's `.Add()` failed *instantly* with a type error. Runs 5/6 are the first to call `.Add($cert)` with a valid **private-key** certificate on the Root store, and both block indefinitely: adding a key-bearing cert to a store location triggers CAPI keyset creation that never completes on headless runners. Fix: trust a **keyless export** (`$cert.Export(X509ContentType.Cert)`) to `CurrentUser\Root`; the private key stays in `My` where `signtool` needs it. Phase markers (`[1/4]`–`[4/4]`) added to the import step so a future stuck run's completed log pinpoints the blocking call.
- Run 32168893884 **failed fast (51 s) at import** — the keyless Root trust fixed the hang: the step now runs in 2.7 s and the phase markers show `[1/4]` → `[2/4]` → `[4/4]` completing in order. New error: `New-Object X509Certificate2 <byte[]>` deconstructs the byte array into 952 scalar constructor arguments ("Cannot find an overload ... argument count: 952"). Fix: construct via `[...X509Certificate2]::new($bytes)`, which takes the `byte[]` as a single argument.
...
2. Job `release-windows` on `windows-latest`, `timeout-minutes: 60`. Steps, in order:
   ...
   - Import Authenticode certificate: decode `WINDOWS_CERT_PFX` (base64) to `$env:RUNNER_TEMP\dms-cert.pfx`, `Import-PfxCertificate` into `Cert:\CurrentUser\My` with `Exportable:$false`, delete the pfx, and assert the expected SHA-1 thumbprint surfaced in the store. If the secrets are missing, the step warns and continues unsigned (dry-run path).
   ...
   - Sign installer with `signtool sign /tr http://timestamp.digicert.com /td sha256 /fd sha256 /sha1 <thumbprint>` (Windows SDK on the runner; searched under both Kits roots), then `signtool verify /pa` as an in-CI proof.
