...
- **Required-now finding:** Intune imported-ADMX assignment for this host was targeted at a user group, not a device group. ADR-0028 and the runbook require Computer Configuration writing `HKLM`; user-group targeting is not the supported path and does not prove managed delivery. Reassign the profile to a device group, wait until Intune reports delivery, then `reg.exe query HKLM\SOFTWARE\Policies\Videoclinic\DMS` without the local fixture. The local `DMSDesktop-test-policy.reg` result remains app-policy evidence only.
...
| 5 | Validate the Windows deployment path and close records | in progress — Phase 4 checkpoint `0deaf8e` confirmed | Windows evidence shows the configured process presents or validates exactly one device-authorization state, `reg.exe query HKLM\SOFTWARE\Policies\Videoclinic\DMS` returns the two expected UUID values, and `cargo test --workspace`, `node --test crates/dms-desktop/ui/*.test.mjs`, and the Windows `Desktop platform smoke` job exit/pass |
