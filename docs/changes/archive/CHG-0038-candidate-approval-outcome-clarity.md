...
**Status:** done — Phase 1 (`c75dbe9`), Phase 2 (`36e0c68`), and the workspace gate passed; archived as the implementation receipt.
...
- The runtime target helper resolves first release, Next minor, Next major, and valid Manual targets into an explicit **What happens next** message that distinguishes direct release from approval-required submission (`crates/dms-desktop/ui/library.mjs`).
- The candidate form uses **Choose requesting editor**, explains that it records the requester rather than the approver, and preselects a group-bound session actor only when its exact object ID appears in `eligible_people` (`crates/dms-desktop/ui/library.mjs`).
...
- Archived CHG-0035 Phase 3 routes the cached verified Entra actor through desktop mutations and bound `WorkspaceSummary.change_author`, so the candidate form can consume that non-secret session projection (`docs/changes/archive/CHG-0035-entra-session-required-for-group-bound-library.md` Phase 3).
- Every `ReleaseRecord` retains immutable `owner`, `editor`, `approver`, and `requester` snapshots; the desktop `CurrentReleaseSelection` now projects them with approval-required and approval-evidence state (`crates/dms-core/src/lifecycle.rs`; `crates/dms-desktop/src/lib.rs`).
- The Library Releases section renders **Immutable release snapshot** with release-time Owner, requester, editor, and approval result; missing historic values are **Unrecorded** and current workflow roles are not substituted (`crates/dms-desktop/ui/library.mjs`).
...
| --- | --- | --- | --- |
| 1 | Make candidate outcome, release identities, and Entra default explicit in the Library | done (`cargo test -p dms-desktop --lib` and `node --test crates/dms-desktop/ui/library.test.mjs` passed) | `node --test crates/dms-desktop/ui/library.test.mjs` exits 0, proving optional, required, first-release, and Manual outcome copy; **Choose requesting editor**; exact actor-ID preselection; editable explicit override; no fuzzy fallback; and distinct immutable release identities |
| 2 | Publish the CAP contracts and matching candidate/release wireframes | done (`node generate.mjs` and 1600×1600 Chrome exports for CAP-0002, CAP-0006, and CAP-0015 passed; visual inspection passed) | `node docs/product/wireframes/generate.mjs` and 1600×1600 headless-Chrome renders of CAP-0002, CAP-0006, and CAP-0015 exit 0; generated screens contain the approved copy |
| 3 | Run the workspace gate and close the change record | done (`cargo fmt`, workspace clippy/test, 128 desktop UI tests, Markdown-link check, and `git diff --check` passed) | `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, `node --test crates/dms-desktop/ui/*.test.mjs`, Markdown-link check, and `git diff --check` all exit 0; CAP/CHG indexes agree and CHG-0038 is archived |
...
1. Run the focused and workspace gates. Fix only regressions caused by this change or existing repository-wide integration defects that prevent a required gate; retain unrelated working-tree changes.
...
3. Correct the pre-existing CHG-0024 Markdown-checker defects: an inline regular-expression character class interpreted as a reference link and an archive-row example resolved relative to `active/`. This is required now because the repository-wide Markdown-link gate otherwise fails before CHG-0038 can close.
4. After every gate passes, record phase evidence, set this CHG to done, move it to `docs/changes/archive/`, and move its README entry from Active to Archive.
