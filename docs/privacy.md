...
| Workspace advisory lock | `<edit-root>/.dms/lock` | OS user, hostname, process id, timestamp; advisory only, never contains document content. A blocked open may show the recorded OS user and hostname locally; that label is not Entra identity and is not sent outbound |
