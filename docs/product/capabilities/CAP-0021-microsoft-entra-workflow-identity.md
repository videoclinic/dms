...
2. First setup and a later binding replacement require an operator to enter the
...
3. A Microsoft 365 administrator supplies a dedicated Entra security group for
...
4. The application signs in interactively to the configured tenant and uses
   ...
   membership.
5. The application refreshes membership before a person is assigned to a role,
...
6. A failed refresh, tenant mismatch, inaccessible group, disabled account, or
...
7. A review request snapshots the effective approver’s Entra tenant/object ID,
...
8. The application stores no Entra password, client secret, or delegated token
...
9. The identity source routes workflow responsibility and verifies an approval
